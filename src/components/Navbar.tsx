...
function Navbar() {
  ...
  const handleClick = () =>{
    ...
    if(currentColor === "#212121"){
      ...
      setTimeout(() => {
        setcurrentColor("#f8f8ff");
      }, 500);
    }
    else{
      setTimeout(() => {
        setcurrentColor("#212121");
      }, 500);
    }
  }
  return (
    ...
    <motion.div  className={`px-5 md:px-12 w-screen backdrop-filter backdrop-blur-sm bg-opacity-5 bg-white fixed py-4 flex items-center  z-50 justify-between transition-transform duration-300 ${
    ...
    <div className="flex items-center gap-7 mt-1">
    {
      ["services", "our work", "about us","insights", "contact us"].map((item, index) => {
        return (
          <div key={index}  className={`hidden relative cursor-pointer  duration-300  overflow-hidden h-7 md:block capitalize ${index === 4 && "ml-64"}  px-1  neue text-[17px]  leading-[26px]`}>
            <motion.div whileHover={{y:-27}} transition={{easings : [0.83, 0, 0.17, 1] , duration:0.3}}  className=" ">
            {item} <br />
    ...
    <IoReorderTwoOutline size={40} color="#727272" />
    </div>
    </motion.div>
    <motion.div initial={{y:-800}} animate={isOpen ? {y:0} : {y: -800} } transition={{easings : [0.25, 1, 0.5, 1] , duration:1}} className=" fixed md:hidden w-screen h-screen bg-[#212121] top-0 left-0 ">

    <div  className= {` absolute w-screen h-[calc(100%-9rem)]  top-36 left-0 border border-[#cbcaca]`}>
      <div>
        {
          ["services", "our work", "about us","insights", "contact us"].map((item, index) => {
            return (
              <motion.div initial={{x:-500}} animate={isOpen ? {x:0} : {x: -500} } transition={{easings : [0.25, 1, 0.5, 1] , duration:index*0.5+1}} key={index} className="text-white border-b-[2px]  grotesk uppercase  text-[11vw] leading-tight  px-5">
                {item}
              </motion.div>
            )
          })
